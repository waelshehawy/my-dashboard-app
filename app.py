            ...
            if not api_key or api_key == "ضع_مفتاحك_هنا":
                ...
                with st.spinner("🧠 جاري تحليل النص ومطابقة الجداول وسياق الـ ERP..."):
                    try:
                        ...
                        قواعد المفاضلة وتحليل النية (Intent Matching) الصارمة:
                        - get_available: إذا طلب اللوحات المتوفرة/المتاحة/الفارغة أو سجلات التجربة. (الشرط المرن في قاعدتك لتجنب الجداول الفارغة: "اسم الزبون" LIKE '%مؤقت%' OR "فترة الحجز" LIKE '%مؤقت%' OR "اسم الزبون" LIKE '%تجربة%' OR "اسم الزبون" IS NULL).
                        - check_temporary: إذا سأل عن الحجوزات المؤقتة، المعلقة، أو سجلات التجربة. (الشرط: "اسم الزبون" LIKE '%مؤقت%' OR "اسم الزبون" LIKE '%تجربة%' OR "فترة الحجز" LIKE '%مؤقت%').
                        - create_package: إذا طلب تجميع لوحات لزبون، عمل باقة عرض سعر لشركة، مع إدراج حسم مالي، أو الفلترة في محافظة معينة.
                        
                        ⚠️ ملاحظة حتمية لمنع الجداول الفارغة: إذا طلب المدير ببساطة "فرجيني لوحات دمشق" أو "عطيني حجوزات حلب" بدون تحديد كلمة متاح أو مؤقت، لا تقم بوضع شروط الفلترة على (اسم الزبون) أو (فترة الحجز) في الـ WHERE clause نهائياً. اكتفِ فقط بالفلترة على اسم المحافظة مثل ("المحافظة" LIKE '%دمشق%') لضمان جلب كافة البيانات الحية المعروضة في السحابة.
