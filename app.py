...
# ============================================================
    ...
    # ==================== قائمة الأحجام (إضافة جديدة) ====================
    st.subheader("📏 تصفح حسب الحجم")
    
    all_sizes = available_data['الحجم'].unique()
    
    # عرض الأحجام في أزرار سريعة
    size_cols = st.columns(min(len(all_sizes), 6))
    for idx, size in enumerate(all_sizes):
        col_idx = idx % len(size_cols)
        with size_cols[col_idx]:
            size_count = len(available_data[available_data['الحجم'] == size])
            if st.button(f"📐 {size}\n({size_count})", key=f"size_filter_{size}", use_container_width=True):
                st.session_state['filter_by_size'] = size
    
    st.divider()
    
    # ==================== فلتر الحجم المختار ====================
    filtered_data = available_data
    if st.session_state.get('filter_by_size'):
        selected_size = st.session_state['filter_by_size']
        filtered_data = available_data[available_data['الحجم'] == selected_size]
        st.info(f"🔍 عرض الأعمدة بحجم: **{selected_size}** ({len(filtered_data)} عمود)")
        
        if st.button("❌ إلغاء التصفية"):
            st.session_state['filter_by_size'] = None
            st.rerun()
        st.divider()
    
    # ==================== عرض المحافظات (كما كانت) ====================
    st.subheader("🏙️ المحافظات")
    
    cities = filtered_data['المحافظة'].unique()
    ...
    for i in range(0, len(cities), cols_per_row):
        ...
        for j, col in enumerate(cols):
            if i + j < len(cities):
                city = cities[i + j]
                city_data = filtered_data[filtered_data['المحافظة'] == city]
            ...
            with st.expander(f"📏 {size_name} - {len(group_data)} عمود", expanded=True):
                ...
                if not valid_coords.empty:
                    ...
                    for _, row in valid_coords.iterrows():
                        popup_text = f"<b>{row['اسم العمود']}</b><br>{row.get('الشبكة', '')}<br>{row['الحجم']}"
                        folium.Marker(
                ...
                st.dataframe(group_data[['رقم اللوحة', 'اسم العمود', 'الشبكة', 'الحجم']], use_container_width=True)
        
        if st.button("🔙 العودة إلى قائمة المحافظات", key="back_to_cities"):
            st.session_state['show_city_details'] = False
            st.rerun()
    
    # ==================== جدول جميع الأعمدة المتاحة ====================
    st.divider()
    st.subheader("📋 جميع الأعمدة المتاحة")
    st.dataframe(
        filtered_data[['رقم اللوحة', 'اسم العمود', 'المحافظة', 'الحجم', 'الشبكة']],
        use_container_width=True,
        hide_index=True
    )
    
    # ==================== الخريطة العامة ====================
    st.subheader("🗺️ خريطة توزع الأعمدة المتاحة")
...
# ============================================================
# صفحة: عرض الفترات (جدول ديناميكي حسب طلب مدير التسويق)
# ============================================================

elif page == "📅 عرض الفترات":
    st.title("📅 عرض الحجوزات حسب الفترات")
    st.info("📌 يعرض هذا الجدول اللوحات والفترات الملونة حسب الحجز")
    ...
    with col_year1:
        selected_year = st.number_input("📅 العام:", min_value=2024, max_value=2030, value=datetime.now().year)
    ...
    # جلب جميع الأعمدة مع معلومات الزبون
    columns_df = pd.read_sql_query("""
        SELECT 
            ...
            b."الحجم",
            b."العدد",
            COALESCE(h."اسم الزبون", '') as customer_name
        FROM "اعمدة انارة" b
        LEFT JOIN "حجوزات1" h ON CAST(b."رقم اللوحة" AS TEXT) = CAST(h."رقم اللوحة" AS TEXT) AND h."العام" = %s
        ORDER BY customer_name, b."رقم اللوحة"
    """, conn, params=(selected_year,))
    
    # جلب الحجوزات للعام المحدد
    ...
    # ============================================================
    # إنشاء قاموس الحجوزات (لوحة → قائمة الفترات)
    # ============================================================
    
    bookings_dict = {}
    for _, row in bookings_df.iterrows():
        board_id = str(row['رقم اللوحة'])
        period = row['فترة الحجز']
        if board_id not in bookings_dict:
            bookings_dict[board_id] = []
        bookings_dict[board_id].append(period)
    
    # ============================================================
    # قائمة الفترات الـ 24 بالترتيب
    # ============================================================
    
    periods_in_order = [
        '1-15 كانون الثاني', '16-31 كانون الثاني',
        '1-15 شباط', '16-28 شباط',
        '1-15 آذار', '16-31 آذار',
        '1-15 نيسان', '16-30 نيسان',
        '1-15 أيار', '16-31 أيار',
        '1-15 حزيران', '16-30 حزيران',
        '1-15 تموز', '16-31 تموز',
        '1-15 آب', '16-31 آب',
        '1-15 أيلول', '16-30 أيلول',
        '1-15 تشرين الأول', '16-31 تشرين الأول',
        '1-15 تشرين الثاني', '16-30 تشرين الثاني',
        '1-15 كانون الأول', '16-31 كانون الأول'
    ]
    
    # ============================================================
    # تجميع البيانات حسب الزبون
    # ============================================================
    
    # ترتيب حسب اسم الزبون (الشركات أولاً)
    columns_df = columns_df.sort_values(['customer_name', 'رقم اللوحة'])
    
    # إضافة صف "إجمالي" في نهاية كل زبون
    # (سنضيفه لاحقاً)
    ...
    st.subheader(f"📊 جدول الحجوزات - عام {selected_year}")
    
    # إعداد البيانات للعرض
    table_data = []
    current_customer = None
    customer_boards = []
    customer_totals = {period: 0 for period in periods_in_order}
    
    for _, row in columns_df.iterrows():
        board_id = str(row['رقم اللوحة'])
        customer = row['customer_name'] if row['customer_name'] else '(بدون زبون)'
        
        # إذا تغير الزبون، أضف صف المجموع السابق
        if current_customer is not None and current_customer != customer:
            # إضافة صف المجموع
            total_row = {
                'الزبون': f"🔹 إجمالي {current_customer}",
                'اللوحة/العمود': '',
                'النوع': 'total'
            }
            for period in periods_in_order:
                total_row[period] = customer_totals[period]
        ...
        # إضافة صف اللوحة
        board_row = {
            'الزبون': customer if current_customer != customer else '',
            'اللوحة/العمود': f"{row['رقم اللوحة']} - {row['اسم العمود']}",
            'النوع': 'board'
        }
        
        for period in periods_in_order:
            if period in bookings_dict.get(board_id, []):
                board_row[period] = '✅'
                customer_totals[period] += 1
            else:
                board_row[period] = ''
        
        table_data.append(board_row)
    ...
    def color_cells(val):
        if val == '✅':
            return 'background-color: #4CAF50; color: white; text-align: center; font-weight: bold;'
        elif isinstance(val, (int, float)) and val > 0:
            return 'background-color: #FFD700; color: #333; text-align: center; font-weight: bold;'
        elif val == '':
    ...
    # تثبيت عمود الزبون واللوحة
    styled_df = styled_df.set_table_styles([
        {'selector': 'th', 'props': [('background-color', '#667eea'), ('color', 'white'), ('padding', '10px'), ('position', 'sticky'), ('top', '0')]},
        {'selector': 'td:first-child', 'props': [('background-color', '#f8f9fa'), ('font-weight', 'bold'), ('position', 'sticky'), ('left', '0')]},
        {'selector': 'td:nth-child(2)', 'props': [('background-color', '#f8f9fa'), ('position', 'sticky'), ('left', '120px')]},
    ])
    
    # عرض الجدول مع تمرير أفقي
    ...
    total_boards = len(columns_df)
    total_customers = columns_df['customer_name'].nunique()
    total_bookings = len(bookings_df)
    
    col1, col2, col3 = st.columns(3)
    col1.metric("🏢 إجمالي اللوحات", total_boards)
    col2.metric("👥 عدد الزبائن", total_customers)
    col3.metric("📅 إجمالي الحجوزات", total_bookings)
    ...
    st.download_button(
        ...
        csv_data,
        f"periods_report_{selected_year}.csv",
        "text/csv",
        use_container_width=True
    )
