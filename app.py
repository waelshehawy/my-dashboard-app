            ...
            if not api_key or api_key == "ضع_مفتاحك_هنا":
                ...
                with st.spinner("🧠 جاري تحليل النص ومطابقة الجداول وسياق الـ ERP..."):
                    try:
                        ...
                        قاعدة صياغة الـ SQL (صارمة وحتمية لمنع تكرار وفقدان السجلات):
                        ...
                        - ⚠️ قاعدة دمج الفترات النصف شهرية الآمنة: لمنع فقدان السجلات التي تحتوي على قيم فارغة NULL في الحجم أو اسم اللوحة، استخدم دائماً دالة COALESCE لتبديل الـ NULL بنص احتياطي، ثم طبق الـ GROUP BY كالتالي:
                          - مثال على الاستعلام التجميعي الآمن والشامل إجبارياً:
                            SELECT "رقم اللوحة", COALESCE("اسم اللوحة", 'غير محدد') AS "اسم اللوحة", "المحافظة", COALESCE("الحجم", 'عام') AS "الحجم", STRING_AGG(DISTINCT "فترة الحجز", ' ، ') AS "فترات الحجز", SUM("اجور عرض") AS "إجمالي اجور العرض" FROM "حجوزات1" WHERE "المحافظة" LIKE '%دمشق%' GROUP BY "رقم اللوحة", COALESCE("اسم اللوحة", 'غير محدد'), "المحافظة", COALESCE("الحجم", 'عام')
