            ...
            if not api_key or api_key == "ضع_مفتاحك_هنا":
                ...
                with st.spinner("🧠 جاري تحليل النص ومطابقة الجداول وسياق الـ ERP..."):
                    try:
                        ...
                        ⚠️ قاعدة إجبارية صارمة للهمزات ومسميات الحقول:
                        - الحقول المخصصة للأجور تُكتب في استعلام الـ SELECT أو الـ WHERE حصراً بألف عادية وبدون همزة قطع كالتالي: ("اجور عرض", "اجور طباعة", "اجور مطبعة")؛ يُمنع منعاً باتاً كتابتها بـ "أجور" بالهمزة لكي لا ينهار سيرفر PostgreSQL السحابي.
                        - استخدم دائماً عامل التشغيل LIKE مع علامات النسبة المئوية (%) للمحافظات والأسماء لضمان جلب البيانات ومنع الشاشات الفارغة (مثل: "المحافظة" LIKE '%دمشق%').
