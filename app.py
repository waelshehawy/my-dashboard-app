            ...
            if not api_key or api_key == "ضع_مفتاحك_هنا":
                ...
                with st.spinner("🧠 جاري تحليل النص ومطابقة الجداول وسياق الـ ERP..."):
                    try:
                        ...
                        1. جدول "حجوزات1":
                           الحقول الدقيقة: "id", "رقم الححز", "اسم الزبون", "رقم الوكيل", "رقم اللوحة", "اجور طباعة", "اجور عرض", "اللوحة", "المحافظة", "رسوم مؤسسة", "شد وتركيب", "عمولات مندوبين", "عمولة مكتب", "فترة الحجز", "العام", "توصيف العمود", "العدد", "اسم الرسم", "الحجم", "user", "اسم اللوحة", "ComputerName", "TimeOfTask", "UserNameOfWin", "الحسم", "اجور مطبعة", "YA", "yyyyy", "logo_path"
                        2. جدول "offers_history":
                           الحقول: id, client_name, offer_date, cart_json, start_p, end_p, year, status
                        
                        ⚠️ قواعد صارمة وحتمية لمنع المسافات العشوائية والأخطاء الإملائية:
                        1. قاعدة الهمزات: الحقول المخصصة للأجور تُكتب في استعلام الـ SELECT أو الـ WHERE حصراً بألف عادية وبدون همزة قطع كالتالي: ("اجور عرض", "اجور طباعة", "اجور مطبعة")؛ يُمنع منعاً باتاً كتابتها بـ "أجور" بالهمزة لكي لا ينهار سيرفر PostgreSQL السحابي.
                        2. قاعدة حقل الحجم (منع المسافات): يُمنع منعاً باتاً وضع أي مسافات فارغة (Spaces) داخل نص الحجم في حقل الـ WHERE. يجب كتابته دائماً بشكل متصل تماماً وبدون أي فراغات (اكتبها دائماً: '2*1' أو '4*3' وليس '2 *1' أو '2 * 1'). وإذا لم يذكر المدير حجماً في طلبه، لا تضع شرط الحجم في الـ WHERE clause نهائياً لضمان جلب السجلات.
                        3. قاعدة البحث المرن: استخدم دائماً عامل التشغيل LIKE مع علامات النسبة المئوية (%) للمحافظات والأسماء لضمان جلب البيانات ومنع الشاشات الفارغة (مثل: "المحافظة" LIKE '%دمشق%').
                        
                        قواعد المفاضلة وتحليل النية (Intent Matching):
                        - get_available: إذا طلب اللوحات المتوفرة/المتاحة/الفارغة أو غير المحجوزة. (الشرط: "اسم الزبون" LIKE '%مؤقت%' أو "فترة الحجز" LIKE '%مؤقت%' أو "اسم الزبون" IS NULL).
                        ...
                        1. أسماء الحقول العربية لجدول "حجوزات1" يجب حتماً وضعها بين علامتي اقتباس مزدوجة "" (مثل: "اسم الزبون"، "المحافظة"). الحقول الإنجليزية تكتب عادية.
