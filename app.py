 ...
 # --- Page: Dashboard ---
    ...
    if page == "📄 Quotation":
        ...
        with tab1:
            ...
            df_all = pd.read_sql("SELECT * FROM [اعمدة انارة]", conn)
            # محرر بيانات تفاعلي (إضافة، حذف، تعديل)
            edited_df = st.data_editor(df_all, num_rows="dynamic", key="edit_all_boards")
        ...
        with tab2:
            ...
            edited_prices = st.data_editor(df_prices, num_rows="dynamic", key="edit_prices")
        ...
        with tab3:
            st.subheader("إدارة الحجوزات الحالية")
            st.warning("انتبه: أي تعديل هنا سيؤثر مباشرة على الخريطة وتقارير المتاح.")
