            ...
            if not api_key or api_key == "ضع_مفتاحك_هنا":
                ...
                with st.spinner("🧠 جاري تحليل النص ومطابقة الجداول وسياق الـ ERP..."):
                    try:
                        ...
                        هيكلية الجداول المتاحة في قاعدة بيانات Supabase:
                        1. جدول "اعمدة انارة" (يحتوي على كافة اللوحات والمواقع):
                           الحقول الدقيقة: "رقم اللوحة", "اسم العمود", "المحافظة", "الشبكة", "الحجم", "العدد"
                        2. جدول "حجوزات1" (يحتوي على تفاصيل الحجوزات والأجور المالية):
                           الحقول الدقيقة: "رقم اللوحة", "فترة الحجز", "العام", "اجور عرض", "شد وتركيب", "اجور طباعة", "TimeOfTask"
                        
                        ⚠️ القواعد الصارمة والقطعية لقاعد صياغة الـ SQL (تطبيقها إجباري):
                        1. قاعدة اللوحات المتاحة (LEFT JOIN): للحصول على اللوحات المتاحة، يجب حتماً عمل LEFT JOIN بين جدول "اعمدة انارة" وجدول "حجوزات1" بناءً على "رقم اللوحة".
                        2. شرط الإتاحة الزمني الصارم: اللوحة متاحة إذا لم يكن لها حجز مستقبلي يغطي تاريخ اليوم (2026-06-05). الحجز يُعتبر منتهياً ومتاحاً إذا كان تاريخ الحجز القديم أقل من تاريخ اليوم أو إذا كان سجل الحجز غير موجود بالأساس (أي حقول جدول "حجوزات1" تعود بـ IS NULL).
                        3. قاعدة حقل الحجم: إذا لم يحدد المدير حجماً معيناً في صوته أو كتابته (مثل 2*1)، لا تقم بحقن شرط الحجم في الـ WHERE Clause نهائياً لضمان جلب كل السجلات. ويُمنع وضع مسافات عشوائية في نص الحجم (اكتبه دائماً متصل '2*1' أو '3*6').
                        4. حقل اسم الزبون: غير مطلوب نهائياً في شروط الفلترة ولا علاقة له بمعادلة الإتاحة، لا تضعه في الـ WHERE مطلقاً.
                        5. الحقول المالية (بدون همزات): تأكد من كتابة أسماء الأعمدة العربية الدقيقة بدون همزات قطع للألف كالتالي: ("اجور عرض", "اجور طباعة", "شد وتركيب") لكي لا ينهار سيرفر PostgreSQL.
                        
                        قواعد النوايا (Intent Matching):
                        - get_available: إذا طلب المدير معرفة اللوحات المتاحة أو الشاغرة أو المتوفرة حالياً في محافظة معينة.
                        - check_temporary: إذا سأل عن تقارير الحجوزات السابقة أو سجلات التجربة.
                        - create_package: إذا طلب تجميع باقة عرض سعر لشركة معينة.
                        
                        قاعدة صياغة الـ SQL الحتمية:
                        - أسماء الجداول العربية والحقول يجب حتماً وضعها بين علامتي اقتباس مزدوجة "" (مثل: FROM "اعمدة انارة" LEFT JOIN "حجوزات1").
                        - المخرجات المطلوبة في الـ SELECT دائماً وبناءً على طلب المدير هي حصراً:
                          SELECT a."اسم العمود" AS "اسم اللوحة", a."العدد", a."الحجم", h."اجور طباعة", h."شد وتركيب" AS "اجور التركيب", h."اجور عرض"
                        
                        يجب أن تطابق المخرجات الهيكل التنفيذي التالي تماماً:
                        {
                            "intent": "get_available / check_temporary / create_package",
                            "confidence": 0.95,
                            "extracted_sql": "استعلام SQL الصحيح والمكتمل بناءً على القواعد الفوقية تماماً",
                            "package_details": null,
                            "spoken_response": "أبشر يا أستاذي، عم جيبلك تفاصيل اللوحات المتاحة وأعدادها وأجورها حالياً فوراً."
                        }
                        """
